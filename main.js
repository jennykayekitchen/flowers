...
const inexpensiveFlowers = () => {
    ...
    // Iterate through each flower, and compare the cost to see if it is less than 2.00 per flower.
    for (const flower of flowers) {
        if (flower.price < 2.00) {
            filteredFlowers.push(flower)
        }
    }
    // Create a new array with all of the flowers that are less than 2.00.
...
/*
    ...
    create a variable for each zone
    iterate through each flower to check if it grows in BOTH zones
    push those flowers to the filterFlowers array
*/
const zoneFlowers = () => {
    const filteredFlowers = []  // Do not modify
        const criteriaZone1 = 3;
        const criteriaZone2 = 8; 
        for (const flower of flowers) {
            if(flower.usdaZones.includes(criteriaZone1) && flower.usdaZones.includes(criteriaZone2)){
                filteredFlowers.push(flower)
            }
        }
...
/*
    ...
    create a variable for orange
    iterate through each flower to check if the colors value contains orange
    push those flowers to the filterFlowers array
*/
const orangeFlowers = () => {
    const filteredFlowers = []  // Do not modify
    const criteriaColor1 = "Orange";
    for (const flower of flowers)
        if(flower.colors.includes(criteriaColor1)){
            filteredFlowers.push(flower)
        }
...
/*
    ...
    for each flower, pull out the color and USDA info
    add the information and tags as strings using += and \n, working down
    ...
    let flowersHTMLString = ""  // Do not modify
        for (const flower of flowers) {
            ...
            for (color of flower.colors) {
                ...
                colorOfFlower = `<section>${color}</section>\n`;
                flowersHTMLString += colorOfFlower
                flowersHTMLString += "\n"
            }

            for(zone of flower.usdaZones) {
                flowersHTMLString +="<h2>USDA Zones</h2>"
                zoneOfFlower = `<div>${zone}</div>\n`
                flowersHTMLString += zoneOfFlower
                flowersHTMLString += "\n"
            }
